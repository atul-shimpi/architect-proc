...
if ( window.location !== window.parent.location ) {
    ...
    if (path === 'sites' || path === 'storage') {
        console.log('User in Preview mode');

        // user on index page
        if ( window.location.pathname.indexOf('storage/projects') === -1 ) {
            console.log('User on home page');
            shopSubdomain = window.location.pathname.split('/')[2];

            //add subdomain as query param to all anchor tag href attribute
            $('a').each(function () {
                console.log('Href = ' + $(this).attr('href'));
                console.log('Changing Href ' + $(this).attr('href') + ' domain ' + shopSubdomain);
                $(this).attr('href', $(this).attr('href') + '?sub-domain=' + shopSubdomain);
            });

        } else {
            // user on non index page
            console.log('User on non index page');
            //http://localhost:8080/storage/projects/4/fHz8ZM23gp6OulHYk5h9iqsktolbg3E1yqzJ/#
            const qryParams = decodeURIComponent(window.location.search.substring(1)).split('=');
            if ( qryParams[0] === 'sub-domain') {
                shopSubdomain = qryParams[1];
                console.log('Subdomain on preview/non index page ' + shopSubdomain);
            }
        }
...
function initMap() {
    ...
    //user in preview mode in production
